...
def draw_dot_vision(node: Node):
    draw_dot = Digraph(comment='The Round Table')
    draw_dot.node(node.name, node.name)
    _draw_dot_vision(draw_dot, node)
    draw_dot.render('test-output/round-table.gv', view=True)
...
def _draw_dot_vision(draw_dot: Digraph, node: Node):
    if node is not None and not node.is_visited:
        ...
        for child in node.children:
                draw_dot.node(child.name, child.name)
                draw_dot.edge(node.name, child.name, node.delta)
...
def reg_ordered_solution(equation: Equation, constants=CONSTANTS, vars=VARS):
    ...
    # работает, но не очень детально
    draw_dot_vision(root_node)


def _reg_ordered_solution(equations: list[Equation], node: Node(list[Equation]), constants=CONSTANTS, vars=VARS, vars_stack=set()):
    ...
    if solved:
        t_node = Node("T", node)
        append_dot_node_with_edge(node, t_node)
        return
    # step 1
    for equation in equations:
    ...
    if left_first_symbol in constants and right_first_symbol in vars:
        ...
        #
        epsilon_node = Node(equations_to_string(replaced_epsilon_equations), new_node, delta=right_first_symbol + "-> eps")
        equation_node = Node(equations_to_string(replaced_equations), new_node, delta=right_first_symbol + "->" + left_first_symbol + right_first_symbol)
        #
